...
public class SignupActivity extends AppCompatActivity {
    ...
    private void saveToDatabase(int type){
        ...
        User user;
        if(type == 0) {
            user = new User(name.getText().toString(),email.getText().toString(),FirebaseAuth.getInstance().getCurrentUser().getUid(),0l,0l,0l);
        } else {
            user = new User(firebaseuser.getDisplayName().toString(),firebaseuser.getEmail().toString(),FirebaseAuth.getInstance().getCurrentUser().getUid(),0l,0l,0l);
        }
        mDatabase.child("Users").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue(user)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void unused) {
                        Common.currentUser = new User(name.getText().toString(),email.getText().toString(), FirebaseAuth.getInstance().getCurrentUser().getUid(),0l,0l,0l);
